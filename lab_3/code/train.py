...
class Mlp:
    ...
    def update_params_batch(self, weight_updates, bias_updates, batch_size, l_rate, gamma):
        for i in range(len(self.weights)):
            update_momentum = gamma * self.prev_weights_updates[i] + l_rate * weight_updates[i] / batch_size
            self.weights[i] -= update_momentum
            self.biases[i] -= l_rate * bias_updates[i] / batch_size
            self.prev_weights_updates[i] = update_momentum
    ...
    def train_batch_v2(self, x_train, y_train, x_test, y_test, num_of_batches=2000, epochs=10, l_rate=0.5, gamma=0.9):
        batches_x = np.array_split(x_train, num_of_batches)
        batches_y = np.array_split(y_train, num_of_batches)
        min_err = math.inf
        weight_cp = np.copy(self.weights)
...
def main():
    ...
    mlp.add_output_layer()
    mlp.train_batch_v2(x_train, y_train, x_test, y_test, epochs=10, num_of_batches=2000)
