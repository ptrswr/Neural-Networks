...
class Mlp:
    def __init__(self, weights_init="Xavier"):
        ...
        # Adagrad
        self.weights_diagonal_mat = []
        self.bias_diagonal_mat = []
    ...
    def update_parameters_adagrad(self, weight_updates, bias_updates, batch_size, l_rate, e=0.00000001):
        self.weights_diagonal_mat += np.power(weight_updates, 2)
        self.bias_diagonal_mat += np.power(bias_updates, 2)
        for i in range(len(self.weights)):
            self.weights[i] -= ((l_rate * weight_updates[i]) / (np.sqrt(self.weights_diagonal_mat[i] + e))) / batch_size
            self.biases[i] -= ((l_rate * bias_updates[i]) / (np.sqrt(self.bias_diagonal_mat[i] + e))) / batch_size
    ...
    def init_parameters(self):
        ...
        self.weights_diagonal_mat = [w * 0 for w in self.weights]
        self.bias_diagonal_mat = [b * 0 for b in self.biases]
    ...
    def train_batch_adagrad(self, x_train, y_train, x_test, y_test, num_of_batches=2000, epochs=10, l_rate=0.5):
        ...
        for i in range(epochs):
            for x_b, y_b in zip(batches_x, batches_y):
                ...
                self.update_parameters_adagrad(weight_updates, bias_updates, len(x_b), l_rate=l_rate)
            print(f"Epoch {i}")
            accuracy = self.predict(x_test, y_test)
            print(f'Accuracy: {accuracy * 100}')
...
def conduct_tests_weights():
    x_train, x_test, y_train, y_test = Mlp.load_data()

    mlp = Mlp(weights_init="He")
    mlp.add_layer(784, 'relu')
    mlp.add_layer(128, 'relu')
    mlp.add_layer(64, 'relu')
    mlp.add_output_layer()
    mlp.train_batch_adagrad(x_train, y_train, x_test, y_test, epochs=10, num_of_batches=2000)

    mlp = Mlp(weights_init="Xavier")
    mlp.add_layer(784, 'sigmoid')
    mlp.add_layer(128, 'sigmoid')
    mlp.add_layer(64, 'sigmoid')
    mlp.add_output_layer()
...
def main():
    # conduct_tests_weights()
    x_train, x_test, y_train, y_test = Mlp.load_data()
    mlp = Mlp(weights_init="Random")
    #
    mlp.add_layer(784, 'relu')
    mlp.add_layer(128, 'relu')
    mlp.add_layer(64, 'relu')
    mlp.add_output_layer()
    mlp.train_batch_adagrad(x_train, y_train, x_test, y_test, epochs=10, num_of_batches=2000, l_rate=0.001)
